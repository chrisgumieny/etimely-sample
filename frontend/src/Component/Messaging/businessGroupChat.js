...
export default function BusinessGroupChat({theme}) {
...
// UseEffect to call the database and get the messages
useEffect(() => {
  ...
  const unsub2 = onSnapshot(getStaffDetails, (QuerySnapshot) => {
    const staffDetails = QuerySnapshot.docs.map((doc) => doc.data());
...
// OnSubmit function to allow user to send message
  const onSubmit = async (values, { setSubmitting }) => {
    ...
    const response = Axios.post(
      `http://localhost:5001/etimely/createMessage/${companyId}`,
      {
